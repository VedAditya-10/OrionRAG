...
server.registerTool(
  "get_workspace_list",
  {
    description: "Retrieve a complete list of all active workspaces (knowledge bases) available in Orion. Use this to find the correct `workspace_id` needed for querying documents or understanding the available context domains.",
  },
  async () => {
    try {
...
async function main() {
  ...
  if (transportType === "http") {
    ...
    app.listen(port, () => {
      console.log(`Orion MCP server running on Streamable HTTP at http://localhost:${port}/mcp`);
    });
    ...
    await server.connect(transport);
    console.error("Orion MCP server running on stdio");
  }
}
