...
Configurable via ORION_EMBEDDING_MODEL in settings.
"""
...
class EmbeddingService:
    ...
    def __init__(self, model_name: Optional[str] = None):
        self.model_name = model_name or settings.ORION_EMBEDDING_MODEL
        self._model = None
    ...
    def model(self):
        ...
        if self._model is None:
            ...
            if "cuda" in device.lower() and not torch.cuda.is_available():
                logger.warning("CUDA is not available. Falling back to CPU for Embedding model.")
                device = "cpu"
