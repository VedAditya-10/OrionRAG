...
class DoclingDocumentParser(BaseDocumentParser):
    ...
    def _get_converter(self):
        ...
        pipeline_options.generate_picture_images = settings.ORION_ENABLE_IMAGE_EXTRACTION
        pipeline_options.images_scale = settings.ORION_DOCLING_IMAGES_SCALE
        pipeline_options.do_formula_enrichment = settings.ORION_ENABLE_FORMULA_ENRICHMENT

        # Set device allocation based on settings
        ...
        if "cuda" in device_setting and torch.cuda.is_available():
            ...
            logger.info("Configuring Docling to use GPU (CUDA)")
        else:
            accel = AcceleratorOptions(device=AcceleratorDevice.CPU)
            logger.info("Configuring Docling to use CPU")
    ...
    def _parse_with_docling(
        ...
        if settings.ORION_ENABLE_TABLE_CAPTIONING and tables:
    ...
    def _chunk_document(
        ...
        chunker = HybridChunker(
            max_tokens=settings.ORION_CHUNK_MAX_TOKENS,
            merge_peers=True,
        )
    ...
    def _extract_images_with_urls(
        ...
        if not settings.ORION_ENABLE_IMAGE_EXTRACTION:
            return [], []
        ...
        for pic in doc.pictures:
            if picture_count >= settings.ORION_MAX_IMAGES_PER_DOC:
        ...
        if settings.ORION_ENABLE_IMAGE_CAPTIONING and images:
