...
class MarkerDocumentParser(BaseDocumentParser):
    ...
    def _get_converter(self):
        ...
        config = {
            ...
            "disable_image_extraction": not settings.ORION_ENABLE_IMAGE_EXTRACTION,
        }

        # LLM-enhanced mode (better tables, equations, handwriting)
        if settings.ORION_MARKER_USE_LLM:
            config["use_llm"] = True
    ...
    def _save_marker_images(
        ...
        if not marker_images or not settings.ORION_ENABLE_IMAGE_EXTRACTION:
            return []
        ...
        for filename, pil_image in marker_images.items():
            if count >= settings.ORION_MAX_IMAGES_PER_DOC:
                break
