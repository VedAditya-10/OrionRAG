...
class BaseDocumentParser(ABC):
    ...
    _IMAGE_CAPTION_PROMPT = (
        ...
        "- Always write the description in English, even if the text in the image is in another language."
    )
    ...
    _TABLE_CAPTION_PROMPT = (
        ...
        "- Always write the description in English, even if the table content is in another language.\n\n"
        "Table:\n"
    )
    ...
    def _caption_tables(self, tables: list[ExtractedTable]) -> None:
        ...
        for tbl in tables:
            ...
            try:
                ...
                if len(table_md) > settings.ORION_MAX_TABLE_MARKDOWN_CHARS:
                    table_md = table_md[:settings.ORION_MAX_TABLE_MARKDOWN_CHARS] + "\n... (truncated)"
