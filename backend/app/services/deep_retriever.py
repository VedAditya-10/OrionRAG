...
Pipeline:
  ...
  2. Vector search → over-fetch top-N candidates (ORION_VECTOR_PREFETCH)
  3. Cross-encoder rerank → precision filter to top-K (ORION_RERANKER_TOP_K)
...
class DeepRetriever:
    ...
    async def query(
        ...
        Flow:
          1. [parallel] KG query + Vector over-fetch (ORION_VECTOR_PREFETCH)
        ...
        prefetch_k = max(settings.ORION_VECTOR_PREFETCH, top_k * 3)
    ...
    async def _kg_query(self, question: str, mode: str) -> str:
        ...
        try:
            return await asyncio.wait_for(
                ...
                timeout=settings.ORION_KG_QUERY_TIMEOUT,
            )
    ...
    def _rerank_chunks(
        ...
        reranked = self.reranker.rerank(
            ...
            documents=doc_texts,
            top_k=top_k,
            min_score=settings.ORION_MIN_RELEVANCE_SCORE,
        )
        ...
        if not reranked:
            ...
            logger.warning(
                ...
                f"{settings.ORION_MIN_RELEVANCE_SCORE}, falling back to top 3"
            )
