...
Configurable via ORION_RERANKER_MODEL in settings.
...
class RerankerService:
    ...
    def __init__(self, model_name: Optional[str] = None):
        self.model_name = model_name or settings.ORION_RERANKER_MODEL
        self._model = None
    ...
    def model(self):
        ...
        if self._model is None:
            ...
            import torch
            device = settings.ORION_RERANKER_DEVICE
            if "cuda" in device.lower() and not torch.cuda.is_available():
                logger.warning("CUDA is not available. Falling back to CPU for Reranker model.")
                device = "cpu"
            logger.info(f"Loading reranker model: {self.model_name} on {device}")
            try:
                self._model = CrossEncoder(self.model_name, device=device, automodel_args={"low_cpu_mem_usage": False})
            except TypeError:
                self._model = CrossEncoder(self.model_name, device=device, model_kwargs={"low_cpu_mem_usage": False})
