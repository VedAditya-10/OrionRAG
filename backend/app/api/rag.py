...
async def query_documents(
    ...
    if isinstance(rag_service, OrionService) and request.mode != "vector_only":
...
async def process_document(
    ...
    if document.status in (DocumentStatus.PROCESSING, DocumentStatus.PARSING, DocumentStatus.INDEXING):
        ...
        timeout = settings.ORION_PROCESSING_TIMEOUT_MINUTES
...
async def get_workspace_rag_stats(
    ...
    # Count ORION documents (parser_version = 'docling')
    orion_result = await db.execute(
        select(func.count(Document.id)).where(
            Document.workspace_id == workspace_id,
            Document.parser_version == "docling"
        )
    )
    orion_documents = orion_result.scalar() or 0
    ...
    return ProjectRAGStatsResponse(
        ...
        orion_documents=orion_documents,
    )
...
async def get_workspace_analytics(
    ...
    # KG analytics (optional — only if ORION active)
    kg_analytics = None
    if orion_documents > 0:
        try:
            kg = await _get_kg_service(workspace_id)
...
async def chat_with_documents(
    ...
    user_parts.append(
        ...
        "- If no source contains relevant information, state that the information is not found in the documents in the same language as the user's question.\n"
    )
...
async def debug_chat(
    ...
    user_parts.append(
        ...
        "- If no source is relevant at all, state that the information is not found in the documents without any citations, in the same language as my question.\n"
    )
