...
function CitationLink({
  ...
  // Vector source — check if it's a timeline source with a timestamp
  let timeLabel = "";
  if (source.heading_path && source.heading_path.length > 0) {
    const lastHeading = source.heading_path[source.heading_path.length - 1];
    const match = lastHeading.match(/\[(\d{2}:\d{2})\]/);
    if (match) {
      timeLabel = match[1];
    }
  }
  ...
  const label = timeLabel
    ? `${docName}-⏱ ${timeLabel}`
    : `${docName}-P.${source.page_no || "?"}`;

  return (
    <span className="inline-flex gap-0.5 mx-0.5 align-middle">
      <button
        onClick={handleContentClick}
        className={`inline-flex items-center gap-0.5 h-[18px] px-1.5 text-[10px] font-medium rounded-full transition-colors whitespace-nowrap ${
          timeLabel
            ? "bg-amber-400/15 text-amber-600 dark:text-amber-400 hover:bg-amber-400/25"
            : "bg-primary/12 text-primary hover:bg-primary/20"
        }`}
        title={`View source: ${doc?.original_filename || "unknown"}${timeLabel ? ` (time: ${timeLabel})` : ` (p.${source.page_no})`}`}
      >
        {timeLabel ? (
...
function InlineImageRef({
  ...
  return (
    <span className="inline-flex flex-col mx-0.5">
      <button
        onClick={handleClick}
        className="inline-flex items-center gap-0.5 h-[18px] px-1.5 text-[10px] font-medium rounded-full bg-orange-400/15 text-orange-600 dark:text-orange-400 hover:bg-orange-400/25 transition-colors align-middle whitespace-nowrap"
