...
Orion combines vector search, knowledge graph, and cross-encoder reranking into one seamless RAG pipeline — powered by Gemini, local Ollama, or fully offline sentence-transformers.
...
Most RAG systems follow a simple pipeline: split text → embed → retrieve → generate. Orion goes further at every stage:

| Aspect | Traditional RAG | Orion |
|---|---|---|
...
Orion supports two document parsers, switchable via `ORION_DOCUMENT_PARSER` env config:
...
ORION_DOCUMENT_PARSER=marker   # or "docling" (default)
```
...
> **Tip**: Gemma 4 models require **Ollama v0.20.0+**. Orion auto-detects native tool calling support — models that support it use Ollama's native tools API (more reliable), others fall back to prompt-based tool calling automatically.
...
Orion was evaluated using two complementary methods: **16 hand-crafted tests** (rule-based metrics) and **30 RAGAS synthetic tests** (LLM-as-judge). Test corpus: TechVina Annual Report 2025 (Vietnamese, 26 chunks) + DeepSeek-V3.2 Technical Paper (English, 57 chunks).
...
```bash
git clone https://github.com/LeDat98/NexusRAG.git OrionRAG-main
cd OrionRAG-main
cp .env.example .env
...
| Variable | Default | Description |
|---|---|---|
| `ORION_EMBEDDING_MODEL` | `BAAI/bge-m3` | Embedding model (1024-dim) |
| `ORION_RERANKER_MODEL` | `BAAI/bge-reranker-v2-m3` | Cross-encoder reranker |
| `ORION_VECTOR_PREFETCH` | `20` | Candidates before reranking |
| `ORION_RERANKER_TOP_K` | `8` | Final results after reranking |
| `ORION_ENABLE_KG` | `true` | Enable knowledge graph extraction |
| `ORION_DOCUMENT_PARSER` | `docling` | Document parser: `docling` (default) or `marker` (lighter, better math) |
| `ORION_MARKER_USE_LLM` | `false` | Enable LLM-enhanced mode for Marker (better tables, equations) |
| `ORION_ENABLE_IMAGE_EXTRACTION` | `true` | Extract images from documents |
| `ORION_ENABLE_IMAGE_CAPTIONING` | `true` | LLM-caption images for search |
| `ORION_KG_LANGUAGE` | `English` | KG extraction language |
...
- [x] **Marker PDF Parser** — Add [Marker](https://github.com/datalab-to/marker) as an alternative document parser with superior math/formula extraction (LaTeX via Surya), lighter GPU footprint (~2-4GB VRAM vs Docling's ~18-20GB), and switchable via `ORION_DOCUMENT_PARSER=marker` env config

---

## MCP Server

Orion includes a [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that exposes its core functionalities:
...
You can directly add the Orion MCP server to Cursor or Claude as an SSE connection using the URL:
...
⭐ If you find Orion useful, please consider giving it a **star** — it helps others discover the project and motivates continued development!
